...
def test_benchmark_runs_same_seeds_and_test_split_for_each_worker_count(monkeypatch):
    ...
    result = module.benchmark(module.BenchmarkConfig(num_configs=30, model_seeds=[0, 1, 2, 3, 4]))
    assert [kwargs["seed_workers"] for _, kwargs in calls] == [1, 2, 5, 5, 2, 1]
    assert all(args == ("seed", "pool", "gold", "test") for args, _ in calls)
